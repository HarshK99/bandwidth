...
- [ ] Compare the browser appearance, if available, to the accepted HTML: current green card, generic guide, compact Coverage toolbar/list, collapsed-area defaults, no area names in time blocks. Include the fixed goal header: compact date/clock/Now row, two goal columns, work-only reminder, no whole-day progress bar. Confirm unchanged header height and visible text on mobile. Fix only concrete differences/usability problems, keeping existing font/components.
