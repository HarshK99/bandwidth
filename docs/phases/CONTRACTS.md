...
Post-rebuild sign-in correction: `getAccessToken(false)` only returns existing unexpired access; otherwise it throws `CalendarReconnectRequired` before loading or calling Google sign-in. The store exposes transient `status: "reconnect"` for that condition and retains cached events. Only Connect and Sync now may request fresh access. Manual sync also loads the calendar list when missing. Google's `prompt: "none"` is not a popup-prevention mechanism and is not used for background renewal.
