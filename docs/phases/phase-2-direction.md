...
- [ ] Adapt the existing timeline, not a copied HTML page: current green card, quiet past/future, next label, generic name/guide, time ruler, subtle progress, and Jump to now. Remove in-card ranges, area/task text, link arrows, and fixed business assignments. Preserve `[data-timeline-box]` for Calendar. Short guides wrap naturally without truncation or increased default card sizes.
...
- [ ] Record data reset, exact files, omitted/performed checks, and the Phase 3 starting point; commit and merge to main_new, then stop.
